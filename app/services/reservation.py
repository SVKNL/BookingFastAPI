...
def create_reservation(db: Session, reservation_data):
    new_reservation_end = reservation_data.reservation_time + timedelta(minutes=reservation_data.duration_minutes)
    ...
    existing_reservation = db.query(ReservationModel).filter(
        ...
        and_(
            ...
            (ReservationModel.reservation_time + (ReservationModel.duration_minutes * func.cast('1 minute', INTERVAL))) > reservation_data.reservation_time,)
    ).all()
